...
export default class Rule {
    ...
    constructor(app: App, title: string) {
        ...
        this.size = new Value(this.id, "SetFontSize", Math.floor(Math.random() * 15) + 30); this.size.active = true;
        ...
        this.alertsound = new Value(this.id, "PlayAlertSound", { id: 1, volume: 150 });
        this.customsound = new Value(this.id, "CustomAlertSound", { file: "testfile" });
        this.icon = new Value(this.id, "MinimapIcon", { size: 0, color: "red", shape: "circle" });
        ...
        this.corruptedmods = new Value(this.id, "CorruptedMods", { sign: '<', value: 0 });
            ...
            if (this[e] && this[e] instanceof Value) {
                if (this[e].name == key) {
                    ...
                    if (key == "PlayAlertSound") {
                        this[e].active = true;
                        this[e].value = { id: parseInt(values[0]), volume: (values[1]) ? parseInt(values[1]) : 150 };
                        return;
                    }
                    //CustomSound type
                    ...
                    if (key == "MinimapIcon" && values.length >= 3) {
                        this[e].active = true;
                        this[e].value = { size: parseInt(values[0]), color: values[1], shape: values[2] };
                        return;
                    }
                    //PlayEffect type
                    if (key == "PlayEffect") {
                    ...
                    if (values.length > 0) {
                        ...
                        values.reduce((p, c) => {
                            ...
                            if (s.trim().length > 0 && !['=='].includes(s.trim())) this[e].value.push(s);
                        })
                        return;
                    }
                    else {
